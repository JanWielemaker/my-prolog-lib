...
%   Modified to work with SWI-Prolog by Jan Wielemaker, 21/1/90
%	* (dynamic)      --> interpreted	(these clauses can be changed)
%	* ttynl          --> nl(user_output)
%	* concat_atoms/3 --> concat/3
...
advised(Goal, Skel, Call) :-
	...
	predicate_property(Skel,interpreted),
...
advise(Goal, _) :-
	display('! You have no interpreted clauses for '),
	display(Goal),
	nl(user_output),
	fail.
