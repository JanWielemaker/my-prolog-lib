...
%	* Deleted public/mode declarations
%	* Inserted module declaration
%	* fileerrors/0 --> fileerrors/2
...
backup(FileName, BackUpExtn) :-
	\+ atom(FileName), !,
	nl(user), display('**error: '),
	display(backup(FileName,'_')), nl(user),
	fail.
backup(FileName, BackUpExtn) :-
	seeing(OldFile),
	fileerrors(OldErrors, off),
	(   see(FileName),
	    seeing(NewFile),
