...
/*  The listing/0 and listing/1 commands are based on the Dec-10
    ...
    The idea of old_portray_clause/1 came from PDP-11 Prolog.
...
old_portray_clause((Pred)) :-
	old_portray_clause((Pred:-true)).
...
/*	Test code for old_portray_clause.
	If it works, test_old_portray_clause(File) should write out the
	contents of File in a more or less readable fashion.

test_old_portray_clause(File) :-
	see(File),
	repeat,
	    read(Clause, Vars),
	    (   Clause = end_of_file
	    ;   test_bind(Vars), old_portray_clause(Clause), fail
	    ),
	!,
	seen.
...
test_bind([X='$VAR'(X)|L]) :-
	test_bind(L).
:-public test_old_portray_clause/1.
*/
